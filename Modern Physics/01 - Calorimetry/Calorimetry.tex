...
Where $m_w$, $m_b$ is the mass of the water and block respectively; $c_w$, $b_w$ is the specific heat capacity of water and block respectively; and $T_{iw}$, $T_{ib}$ is the initial temperature of the water and block respectively.
...
\begin{equation}
    \begin{split}
        \text{percent error} &= \left( \frac{T_{f-exp} - T_{f-theory}}{T_{f_theory}} \right) 100 \\
        \text{percent error} &= \SI{-0.0153
        }{\percent}
    \end{split}
\end{equation}

Using the approach above, we put into a table below, the experimental equilibrium temperatures from multiple experiments along with the corresponding theoretical temperatures and percent error between the two.
